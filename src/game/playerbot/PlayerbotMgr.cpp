...
PlayerbotMgr::PlayerbotMgr(Player* const master) : m_master(master)
{
    ...
    m_confCollectDistanceMax = sWorld.getConfig(CONFIG_FLOAT_PLAYERBOT_COLLECT_MAX_DISTANCE);
    if (m_confCollectDistanceMax > 100)
    {
        sLog.outError("Playerbot: PlayerbotAI.Collect.DistanceMax higher than allowed, check your Configure file. . Using 100");
        m_confCollectDistanceMax = 100;
    }

    m_confCollectDistance = sWorld.getConfig(CONFIG_FLOAT_PLAYERBOT_COLLECT_DISTANCE);
    if (m_confCollectDistance > m_confCollectDistanceMax)
    {
        sLog.outError("Playerbot: PlayerbotAI.Collect.Distance higher than Playerbot.Collect.DistanceMax, check your Configure file. Using DistanceMax value");
        m_confCollectDistance = m_confCollectDistanceMax;
    }

    gConfigSellLevelDiff = sWorld.getConfig(CONFIG_UINT32_PLAYERBOT_SELLALL_LV_DIFF);
