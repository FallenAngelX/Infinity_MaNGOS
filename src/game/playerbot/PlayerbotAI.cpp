...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    // forced to change target to current target == nullptr operation
...
void PlayerbotAI::MovementReset()
{
    ...
    if (m_movementOrder == MOVEMENT_FOLLOW)
    {
        ...
        Player* pTarget = nullptr;                            // target is player
...
// Testing
        if (m_bot->isAlive() && !m_bot->IsBeingTeleported() && (m_bot->GetMap() == m_followTarget->GetMap()))
        {
            if (DistOverRide != 0)
            {
                if (m_FollowAutoGo == FOLLOWAUTOGO_OFF)
...
uint32 PlayerbotAI::TalentSpecDBContainsError()
{
    ...
    do
    {
        ...
        */
        ...
        if (!ts_id)    // Nice bit of paranoia: ts_id is a non-zero NOT nullptr AUTO_INCREMENT value
...
void PlayerbotAI::_HandleCommandFollow(std::string &text, Player &fromPlayer)
{
    ...
    SetMovementOrder(MOVEMENT_FOLLOW, GetMaster());
    SendWhisper("I Will Follow", fromPlayer);
}
...
void PlayerbotAI::_HandleCommandStay(std::string &text, Player &fromPlayer)
{
    ...
    SendWhisper("I Will Stay Right Here.", fromPlayer);
}
